...
class TOTAL3D(BaseNetwork):
    ...
    def __init__(self, cfg):
        ...
        print(f'network has {phase_names}')
        ...
        # FIX(taesu) : Multi-GPU not worked
        # '''Multi-GPU setting'''
    ...
    def get_extra_results(self, all_output):
        ...
        extra_results = {}
        # FIX(taesu) : "module" in keyname of each layer is removed after torch update
        # if isinstance(self.mesh_reconstruction.module, MODULES.get('LDIF')):
        if isinstance(self.mesh_reconstruction, MODULES.get('LDIF')):
